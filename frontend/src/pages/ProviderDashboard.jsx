...
// Components
import Sidebar from "../components/ ProviderComponents/ProviderSidebar";
import Header from "../components/ ProviderComponents/Header";
import Footer from "../components/ ProviderComponents/Footer";
import Overview from "../components/ ProviderComponents/Overview";
import TotalTransactions from "../pages/TotalTransactions";
import AddPlan from "../components/ ProviderComponents/AddPlan"; // ✅ Import AddPlan
...
function ProviderDashboard() {
  ...
  const [platformFeeReceived, setPlatformFeeReceived] = useState(
    Number(localStorage.getItem("platformFeeReceived") || "0")
  );
  const [totalEnergySupplied, setTotalEnergySupplied] = useState(
    Number(localStorage.getItem("providerEnergySupplied") || "0")
  );
  ...
  useEffect(() => {
    const onStorageUpdate = () => {
      ...
      setTotalEnergySupplied(
        Number(localStorage.getItem("providerEnergySupplied") || "0")
      );
      setPlatformFeeReceived(
        Number(localStorage.getItem("platformFeeReceived") || "0")
      );
    };
  ...
  const renderContent = () => {
    switch (activeSection) {
      case "overview":
        return (
          <Overview
            platformFeeReceived={platformFeeReceived}
            totalEnergySupplied={totalEnergySupplied}
      ...
      case "total_transactions":
        return <TotalTransactions transactions={transactions} />;

      case "add_plans": // ✅ New case
        return <AddPlan showToast={(msg, type) => alert(`${type}: ${msg}`)} />;
  ...
  const pageTitles = {
    overview: "Provider Dashboard Overview",
    total_transactions: "All Transactions",
    add_plans: "Manage Plans", // ✅ Title for AddPlan
  };
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans">
      ...
      <div className="flex-1 flex flex-col ml-64 lg:ml-0 h-screen overflow-auto">
        <Header
          onMenuClick={() => setSidebarOpen(!sidebarOpen)}
          title={pageTitles[activeSection]}
          notificationCount={notificationCount}
          onNotificationsClick={() => setActiveSection("overview")}
        />
